...
class Perspective3DEngine:
    ...
    def __init__(self, frame_w=cfg.FRAME_WIDTH, frame_h=cfg.FRAME_HEIGHT):
        ...
        # X: -1.5m to +1.5m, Y: 0.0m to 20.12m
    ...
    def pixel_to_world_2d(self, px, py):
        """
    ...
    def world_to_pixel_2d(self, X, Y):
        """
    ...
    def estimate_3d_point(self, px, py, radius_px):
        """
        Estimates full (X, Y, Z) world point from 2D pixel coordinates and ball radius.
        Uses ball radius perspective scale to infer vertical height Z.
        """
        X, Y = self.pixel_to_world_2d(px, py)

        # Base ball radius at bowler end (~0.036m = ~5px) to batsman end (~0.036m = ~15px)
        t = Y / PITCH_LENGTH_M
        t = max(0.0, min(1.0, t))
        expected_ground_r = cfg.MIN_BALL_RADIUS + t * (cfg.MAX_BALL_RADIUS - cfg.MIN_BALL_RADIUS)

        # Height scale factor in metres per pixel at depth Y
        m_per_pixel = 3.0 / (cfg.STUMPS_WIDTH_FAR + t * (cfg.STUMPS_WIDTH_NEAR - cfg.STUMPS_WIDTH_FAR))

        # Height Z is estimated from perspective displacement / radius scaling
        delta_r = radius_px - expected_ground_r
